...
Vite + Tailwind CSS frontend. Local persistence via a portable `config.json`
(no database).
...
- `pkg/manager` — orchestrates config + libraries.
...
- **`config.json` is the source of truth** for configured library paths and
  host-level settings. Package metadata is rebuilt from disk scans in memory.
