...
import { Network, AlertTriangle, ExternalLink, Play, Square, Globe } from 'lucide-react';
        ...
        >
            <div className="flex items-start justify-between">
                ...
                <button
                    ...
                    className={clsx(
                        "px-3 py-1.5 rounded-lg text-xs font-mono border transition-all flex items-center gap-2",
                        isRunning
                            ? "bg-blue-500/10 text-blue-400 border-blue-500/30 hover:bg-blue-500/20 cursor-pointer shadow-lg shadow-blue-900/10"
                            : "bg-gray-800/50 text-gray-600 border-gray-700 cursor-not-allowed opacity-75"
                    )}
                    title={isRunning ? "Open in Browser" : "Server Stopped"}
                >
                    <Globe size={14} className={isRunning ? "opacity-100" : "opacity-50"} />
                    <span>http://{localIP}:{serverPort}</span>
                    {isRunning && <ExternalLink size={12} className="opacity-50" />}
                </button>
            </div>
