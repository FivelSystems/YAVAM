...
- **Release Pipeline**: Reworked the release process around the new Stable and Unstable channels.

### Fixed
- **Links No Longer Break the App**: Clicking a web link (for example in the "What's New" window) now opens it in your system browser instead of navigating inside the app. Previously such a link took over the app window with no way to go back, and the app had to be restarted to recover.
...
- **Stable Dependency List**: The details panel now lists a package's **direct declared dependencies** in a fixed order that no longer changes when you switch libraries — only each entry's status updates (present here, found in another library, or missing). Previously it rendered a library-relative resolved *tree* whose contents and order shifted every time you changed libraries.
